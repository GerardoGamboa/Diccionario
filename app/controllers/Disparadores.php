...
class Disparadores extends Controller {
    ...
    public function add() {
        if($_SERVER['REQUEST_METHOD'] == 'POST') {
            ...
            $data = ['tabla_id' => trim($_POST['tabla_id']), 'nombre' => trim($_POST['nombre']), 'evento' => trim($_POST['evento']), 'definicion' => trim($_POST['definicion']), 'descripcion' => trim($_POST['descripcion']), 'user_id' => $_SESSION['user_id'], 'nombre_err' => ''];
            ...
            $data = ['tablas' => $this->tablaModel->getTablas(), 'tabla_id' => '', 'nombre' => '', 'evento' => '', 'definicion' => '', 'descripcion' => ''];
    ...
    public function edit($id) {
        if($_SERVER['REQUEST_METHOD'] == 'POST') {
            $_POST = filter_input_array(INPUT_POST, FILTER_SANITIZE_FULL_SPECIAL_CHARS);
            $data = ['id' => $id, 'tabla_id' => trim($_POST['tabla_id']), 'nombre' => trim($_POST['nombre']), 'evento' => trim($_POST['evento']), 'definicion' => trim($_POST['definicion']), 'descripcion' => trim($_POST['descripcion']), 'nombre_err' => ''];
            if(empty($data['nombre'])) $data['nombre_err'] = 'Ingrese nombre';
            if(empty($data['nombre_err'])) {
            ...
            $data = ['id' => $id, 'tablas' => $this->tablaModel->getTablas(), 'tabla_id' => $disp->tabla_id, 'nombre' => $disp->nombre, 'evento' => $disp->evento, 'definicion' => $disp->definicion, 'descripcion' => $disp->descripcion];
