...
class Procedimiento {
    ...
    public function addProcedimiento($data) {
        ...
        if($this->db->execute()){
            ...
            return $this->addVersion($objeto_id, 'procedimiento', $data);
        } else {
            return false;
        }
    }
    ...
    public function updateProcedimiento($data) {
        ...
        if($this->db->execute()){
            return $this->addVersion($data['id'], 'procedimiento', $data);
        } else {
            return false;
        }
    }
    ...
    public function getVersions($id){
        $this->db->query('SELECT v.*, u.name as creatorName FROM dic_versiones v LEFT JOIN dic_usuarios u ON v.usuario_creador = u.id WHERE objeto_id = :objeto_id AND tipo_objeto = "procedimiento" ORDER BY consecutivo DESC');
